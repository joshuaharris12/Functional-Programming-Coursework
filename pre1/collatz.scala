...
def collatz_max(bnd: Long) : (Long, Long) = {
    val stepsList = for(n <- (1L to bnd)) yield (collatz(n), n)
    val maxPair = stepsList.maxBy(_._1)
    (maxPair._1, maxPair._2)
}
...
def is_pow_of_two(n: Long) : Boolean = if ( (n & (n - 1L)) == 0L) true else false

def is_hard(n: Long) : Boolean = if (is_pow_of_two( (3L * n) + 1L) ) true else false

def last_odd(n: Long) : Long = {
    if (is_hard(n)) n
    else 
        if ((n % 2L == 0)) last_odd(n / 2L) else last_odd( (n * 3L) + 1L )
}
