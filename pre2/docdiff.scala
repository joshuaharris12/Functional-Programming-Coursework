...
def clean(s: String) : List[String] = {
    val rExpression = """\w+""".r
    rExpression.findAllIn(s).toList
}
...
def occurrences(xs: List[String]): Map[String, Int] = {
   val wordsSet = xs.toSet
   val lst = for (word <- wordsSet.toList) yield {
       (word, xs.count(x => x == word))
   }
   lst.toMap 
}
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    ...
    val dProduct = for ( (key, value) <- map1 ) yield {
        try {
            map2.get(key).get * value
        } catch {
            case e : Exception => 0 * value
        }
    }
    dProduct.sum
}
...
def overlap(lst1: List[String], lst2: List[String]) : Double = {
    val prodResult = prod(lst1, lst2)
    val dSqaureList = List(prod(lst1, lst1), prod(lst2, lst2))
    prodResult.toDouble / dSqaureList.max.toDouble
}
